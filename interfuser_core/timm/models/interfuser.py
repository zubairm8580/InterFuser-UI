...
class HybridEmbed(nn.Module):
    def __init__(
        ...
        print('[INTERCEPT] Backbone forced: RGB=r50, LiDAR=r18')
        super().__init__()
        use_different_backbone = True
        rgb_backbone_name = 'r50'
        lidar_backbone_name = 'r18'
        print('[LOGIC FIX] Forced: Split=True, RGB=r50, LiDAR=r18')
...
class Interfuser(nn.Module):
    def __init__(
        ...
        dim_feedforward=2048,
        normalize_before=False,
        rgb_backbone_name="r50",       # [수정 1] r26 -> r18 (512채널로 변경)
        lidar_backbone_name='r18',     # [수정 2] r26 -> r18
        num_heads=8,
        norm_layer=None,
        dropout=0.1,
        end2end=False,
        ...
        waypoints_pred_head="gru",     # [수정 3] heatmap -> gru (size 5 -> 11로 변경)
        reverse_pos=True,
        use_different_backbone=True,
        use_view_embed=True,
